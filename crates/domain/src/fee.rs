...
//!
//! 对应策略风险修复项 #1：策略原文把 Taker 4% 写死，本模型将其抽象为可配置参数，
//! 避免在策略逻辑中硬编码任何费率。Polymarket 官方文档未公开费率，实测 Taker 约 3% 多，
//! 统一按 4% 计提作为略偏保守的口径。费率经由配置文件加载，后续变化只需改配置即可一键调整。
//!
//! **扣费口径**（实测确认）：Polymarket 的手续费体现为**到手股数的扣减**，而非额外扣减现金。
//! 即下单 `gross_qty` 股，实际入仓 `gross_qty × (1 - rate)` 股，所花现金仍为 `gross_qty × price`。
//! 交割时每股兑付 1 美元，故股数损耗直接决定回收金额——这与按金额另收手续费在 PnL 上并不等价。
