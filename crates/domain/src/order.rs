...
/// 交易所对单笔订单的最小量约束。
///
/// 对应策略风险修复项 #7。真实交易所对单笔订单有最小下单门槛与精度限制：
/// - 最小份数：单笔至少买卖若干份代币；
/// - 最小金额：单笔名义金额（份数 × 价格）至少若干美元；
/// - 价格 / 数量精度：价格与份数各自限制到固定小数位（超出会被拒）。
///
/// 最小份数与最小金额两个约束在不同价位下松紧不同，故须同时满足：以两者的临界价
/// `min_notional / min_order_size` 为分界——价格高于该临界价时份数约束更紧，低于时金额约束更紧
/// （按默认值 1 美元 / 5 份，临界价为 0.2）。布阵、风控、模拟撮合统一复用本类型校验与量化。
